...
# Composite stages: ngram-mod (self-spec) + MTP (traditional spec)
# Usage: bash ~/Desktop/run-ik-dual-spec.sh

~/Desktop/ik_llama.cpp-pr1789/build/bin/llama-server \
  ...
  --spec-stage mtp:n_max=15,draft-p-min=0.0 \
  --no-mmap \
  -np 1
